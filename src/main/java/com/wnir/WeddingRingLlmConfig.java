...
public final class WeddingRingLlmConfig {
    ...
    public static int     contextWindow      = 40960;
    public static int     memoryBudgetTokens = 8192;
    public static int     maxResponseTokens  = 1024;
    public static int     thinkingBudget     = 1024;
    ...
    private static final String DEFAULT_CONTENT = """
        ...
        # temperature — 0.0–1.0
        # context_window — MUST match --ctx-size used when starting llama.cpp.
        #   Set to n_ctx_train reported by the model (never exceed it).
        #   Mismatch causes silent context truncation and incoherent responses.
        # memory_budget_tokens — token budget for system prompt + memory + todo.
        #   Remaining (context_window - memory_budget_tokens) goes to history.
        #   8192 is enough for the system block; most of the window stays for history.
        ...
        try {
            for (String raw : Files.readAllLines(path)) {
                ...
                switch (k) {
                    ...
                    case "thinking_budget"       -> { try { thinkingBudget = Integer.parseInt(v); } catch (Exception ignored) {} }
                }
            }
        } catch (IOException e) {
